...
function create(id, esm2, parent, options = {}, states) {
  ...
  Object.defineProperty(esm2, `${specialKeys.resolved}`, {
    value: new Promise((resolve3) => isResolved = async () => {
      resolve3(true);
    }),
    writable: false,
    enumerable: false
      ...
      for (let key in attributes2) {
        if (key === "style") {
          ...
          if (isEventListener(key, value2)) {
            ...
            const valueToSet = handleAttribute(key, value2, esm2);
            esm2[specialKeys.element].setAttribute(key, valueToSet);
            try {
              esm2[specialKeys.element][key] = valueToSet;
            } catch (e) {
            }
            ;
          }
...
async function asyncConnect(keys, onReadyCallback) {
  ...
  for (let name2 in this[keys.hierarchy]) {
    ...
    if (typeof init === "function")
      ...
      console.error(`Could not start component ${name2} because it does not have a __onconnected function`);
  }
  this[`__${keys.resolved}`]();
  if (onReadyCallback)
...
function stop_default(keys) {
  ...
  if (this[keys.hierarchy]) {
    for (let name2 in this[keys.hierarchy]) {
      ...
      if (typeof component[keys.stop] === "function")
        ...
        console.warn("Could not disconnect component because it does not have an __ondisconnected function", name2, this.__children);
    }
  }
  ...
  const privateEditorKey = `${keys.editor}Attached`;
  if (this[privateEditorKey])
...
var create2 = (config, toMerge = {}, options = {}) => {
  ...
  const onConnected = (instance) => {
    ...
    if (noParent)
      ...
      const startRes = instance[specialKeys.start]();
      return resolve(startRes, resolve2);
    }
  };
